#include "infinite_grid_render_system.h"
...
namespace arc
{
	...
	void cInfiniteGridRenderSystem::createPipeline(VkRenderPass _render_pass)
	{
		...
		arc_pipeline = std::make_unique<arcPipeline>(
			arc_device,
			pipeline_config,
			"F:/Arcanum/src/Shaders/compiled_shaders/infinite_grid_shader.vert.spv",
			"F:/Arcanum/src/Shaders/compiled_shaders/infinite_grid_shader.frag.spv"
		);
	}
