...
void ArcEngine::VulkanGraphics::init_imgui(std::shared_ptr<Window> _window)
{
    ...
    info.PipelineRenderingCreateInfo.pColorAttachmentFormats = &swapchain._swapchainImageFormat;
    info.MSAASamples = VK_SAMPLE_COUNT_1_BIT;

    init_info.PipelineInfoMain = info;
    init_info.PipelineInfoForViewports = info;
...
void ArcEngine::VulkanGraphics::draw_imgui(VkCommandBuffer cmd, VkImageView targetImageView)
{
    VkRenderingAttachmentInfo colorAttachment = vkinit::attachment_info(targetImageView, nullptr, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
    VkRenderingInfo renderInfo = vkinit::rendering_info(swapchain._swapchainExtent, &colorAttachment, nullptr);
...
void ArcEngine::VulkanGraphics::RenderMesh(const Model& _Model, const glm::mat4x4& _ObjectToWorld)
{
    ...
    VK_CHECK(vkAcquireNextImageKHR(_device, swapchain._swapchain, 1000000000, get_current_frame()._swapchainSemaphore, nullptr, &swapchainImageIndex));
    ...
    vkutil::transition_image(cmd, swapchain._swapchainImages[swapchainImageIndex], VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);

    // execute a copy from the draw image into the swapchain
    vkutil::copy_image_to_image(cmd, _drawImage.image, swapchain._swapchainImages[swapchainImageIndex], _drawExtent, swapchain._swapchainExtent);

    // set swapchain image layout to Attachment Optimal so we can draw it
    vkutil::transition_image(cmd, swapchain._swapchainImages[swapchainImageIndex], VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);

    //draw imgui into the swapchain image
    draw_imgui(cmd, swapchain._swapchainImageViews[swapchainImageIndex]);

    // set swapchain image layout to Present so we can draw it
    vkutil::transition_image(cmd, swapchain._swapchainImages[swapchainImageIndex], VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR);
...
void ArcEngine::VulkanGraphics::draw_background(VkCommandBuffer cmd)
{
    ...
    ////clear image
    //vkCmdClearColorImage(cmd, swapchain._swapchainImages[swapchainImageIndex], VK_IMAGE_LAYOUT_GENERAL, &clearValue, 1, &clearRange);
...
void ArcEngine::VulkanGraphics::init_descriptors()
{
...
void ArcEngine::VulkanGraphics::init_pipelines()
{
    init_background_pipelines();
}

void ArcEngine::VulkanGraphics::init_background_pipelines()
{
    ...
    if (!vkutil::load_shader_module("../Shaders/ShadersVulkan/Source/VulkanShadersCompiled/test.comp.spv", _device, &computeDrawShader))
    {
