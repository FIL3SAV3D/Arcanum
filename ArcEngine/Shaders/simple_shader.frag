...
void main()
{
	...
	vec3 sampledColor = texture(texSampler, fragUV).rgb;
	...
	for(int i = 0; i < ubo.numLights; i++)
	{
		...
		float blinnTerm = dot(normalize(sampledNormals), halfAngle);
		blinnTerm = clamp(blinnTerm, 0.0, 1.0);
		blinnTerm = pow(blinnTerm, 512.0); // higher values -> sharper highlight

		specularLight += intensity * blinnTerm;
	}
	outColor = vec4(diffuseLight * sampledColor + specularLight * sampledORM.b, 1.0);
//	outColor = vec4(texture(ORMTexSampler, fragUV).rgb, 1.0);
}